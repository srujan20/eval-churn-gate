...
first segment pays for loading scikit-learn and the pacing of the whole video is
wrong.
...
WARMUP = [sys.executable, "-m", "churngate", "--version"]

# Ordered as the argument, not as the feature list: the baseline, then the thing
# every demo shows, then the number no other tool reports, then the corpus, then
# the uncomfortable posture, then the receipts.
SEGMENTS = (
    ("What this eval set can resolve, before running any comparison", ["power"]),
    (
        "A slice collapses and the aggregate does not move. Exit 1",
        ["gate", "--scenario", "compensating"],
    ),
    (
        "The gate almost everyone ships, on the same comparison. It passes",
        ["one", "--gate", "mean_threshold", "--scenario", "compensating"],
    ),
    (
        "The same version twice. The honest answer is that it cannot tell",
        ["gate", "--scenario", "aa", "--seed", "7"],
    ),
    (
        "Every gate on every scenario, with an interval on every rate",
        ["sweep", "--replications", "120"],
    ),
)
...
def capture(label: str, argv: list[str], *, module: bool) -> Captured:
    command = f"python -m churngate {' '.join(argv)}" if module else f"python {' '.join(argv)}"
    full = [sys.executable, "-m", "churngate", *argv] if module else [sys.executable, *argv]
...
def main() -> int:
    ...
    print("warming the import so the first segment is not paying for scikit-learn")
