...
FOOTER = (
    ...
    "<span>Srujan Sadineni  |  eval-churn-gate</span>"
    '<span class="pageNumber"></span></div>'
)
HEADER = '<div style="font-size:0"></div>'

COVER_ROWS = (
    ("Regressions the shipped aggregate gate catches", "mean_threshold_detection", ""),
    ("Regressions the combined gate catches", "combined_detection", ""),
    ("What that costs on a clean release", "combined_false_block", ""),
    ("A collapsed slice, seen by the aggregate gate", "mean_threshold_on_compensating", ""),
    ("The same slice, seen by the slice scan", "slice_scan_on_compensating", ""),
    ("Items flipped by run to run noise alone", "churn_floor_median", ""),
    ("Of that movement, cancelled by the net", "median_hidden_share", ""),
    ("Rank correlation of the delta with the churn", "correlation", ""),
    ("Churn across the same near zero delta, lowest", "flip_min", ""),
    ("Churn across the same near zero delta, highest", "flip_max", ""),
    ("Smallest detectable regression, paired", "mde_paired", ""),
    ("The same, comparing two averages", "mde_unpaired", ""),
    ("A regression of exactly the threshold size, caught", "coin_flip", ""),
    ("Comparisons where the honest answer is I cannot tell", "undecidable_share", ""),
...
<h1>eval-churn-gate</h1>
<p class="subject">Defense guide. An aggregate score delta is a function of two
numbers, and two numbers cannot distinguish nothing changing from half the items
improving while half regress. This is what that costs, measured.</p>
